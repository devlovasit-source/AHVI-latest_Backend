...
PHYSICAL_OBSERVATION_MIN_CONFIDENCE = 0.60
...
def map_physical_garment_observations(
    observations: Any,
    *,
    min_confidence: float = PHYSICAL_OBSERVATION_MIN_CONFIDENCE,
) -> Dict[str, Any]:
    """
...
def build_climate_profile(
    ...
    *,
    ...
    profile: Dict[str, Any] = {
        k: climate_unknown_tuple() for k in keys
    }

    # 1. Carry forward existing evidence.
    if existing_profile:
    ...
    # 2. Add deterministic evidence from the stored garment record.
    profile = merge_climate_profile(
        profile,
        derive_deterministic_climate_properties(
            item,
    ...
    # 3. Add currently proven vision evidence.
    profile = merge_climate_profile(
        profile,
    ...
    # 4. Add physical garment observations from the dedicated
    #    physical analysis service.
    physical_properties = map_physical_garment_observations(
        physical_observations
    )

    for key, candidate in physical_properties.items():
        profile[key] = merge_climate_value(
            profile.get(key),
            candidate,
        )
