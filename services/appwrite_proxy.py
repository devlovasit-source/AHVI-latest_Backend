...
class AppwriteProxyError(Exception):
    """Appwrite transport/API failure. status_code carries the HTTP status
    for API-level errors (e.g. 404 not-found, 409 conflict) and is None for
...
class AppwriteProxy:
    ...
    def _ensure_config(self) -> None:
        ...
        if not self.api_key:
            missing.append("APPWRITE_API_KEY/APPWRITE_KEY")
