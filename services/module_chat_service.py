...
async def handle_planner_chat(message: str, context: Dict[str, Any], user_id: str) -> Dict[str, Any]:
    from brain.plan_pack_flow import build_plan_pack_response
    res = build_plan_pack_response(message, context=context)
    return _envelope(
        domain="planner",
        message=res.get("message") or "Built your packing checklist.",
        chips=res.get("chips"),
        cards=res.get("cards"),
        data=res,
    )
...
async def handle_module_chat(payload: Dict[str, Any], user_id: str = "") -> Dict[str, Any]:
    ...
    if domain in ("planner", "plan_pack") or any(w in message.lower() for w in ("pack", "carry-on", "packing")):
        return await handle_planner_chat(message, context, user_id)
    if domain == "diet":
