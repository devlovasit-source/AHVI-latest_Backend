...
def get_intelligent_occasions_for_item(name: object = "", category: object = "", sub_category: object = "") -> List[str]:
    """Returns domain-aware intelligent occasion suggestions for a garment."""
    blob = " ".join(str(x or "").lower() for x in (name, category, sub_category))

    # Ethnic / Traditional Indian Wear
    if any(k in blob for k in ("lehenga", "saree", "sari", "sherwani", "kurta", "kurti", "anarkali", "dupatta", "indo-western", "ethnic", "traditional")):
        return ["Weddings", "Festive", "Functions", "Traditional Events", "Cultural", "Reception"]

    # Beachwear / Swimwear
    if any(k in blob for k in ("beach", "swim", "bikini", "monokini", "sarong", "coverup", "boardshort")):
        return ["Beach Outings", "Poolside", "Resort Wear", "Vacation"]

    # Activewear / Gym
    if any(k in blob for k in ("gym", "sport", "active", "workout", "running", "legging", "track", "athletic")):
        return ["Gym & Workout", "Sports", "Fitness", "Activewear"]

    # Formal / Office
    if any(k in blob for k in ("blazer", "suit", "trouser", "oxford", "formal", "business", "office", "work")):
        return ["Office", "Business Meeting", "Formal Event", "Presentation"]

    # Party / Evening
    if any(k in blob for k in ("party", "gown", "sequin", "cocktail", "night out", "club")):
        return ["Party", "Night Out", "Dinner Party", "Cocktail Event"]

    # Casual / Everyday
    return ["Casual", "Daily Wear", "Weekend", "Outing"]
...
__all__ = (
    ...
    "get_intelligent_occasions_for_item",
)
