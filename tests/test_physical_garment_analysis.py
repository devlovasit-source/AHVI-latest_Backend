...
# ---------------------------------------------------------------------------
# Confidence summary & single canonical confidence threshold
# ---------------------------------------------------------------------------
...
def test_canonical_confidence_threshold_constant():
    assert hasattr(pgas, "PHYSICAL_ANALYSIS_MIN_CONFIDENCE")
    assert pgas.PHYSICAL_ANALYSIS_MIN_CONFIDENCE == 0.60


def test_material_family_candidates_preserves_structure_and_provenance():
    raw = _valid_output(
        material_family_candidates=[
            {
                "value": "cotton_like",
                "confidence": 0.85,
                "provenance": "vision:ollama",
                "context": "slub texture",
            }
        ]
    )
    ...
    result = pgas.validate_analysis_output(raw)
    assert result is not None
    candidates = result["material_family_candidates"]
    assert len(candidates) == 1
    assert candidates[0]["value"] == "cotton_like"
    assert candidates[0]["confidence"] == 0.85
    assert candidates[0]["provenance"] == "vision:ollama"
    assert candidates[0]["context"] == "slub texture"
