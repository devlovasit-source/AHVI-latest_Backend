...
_MODULE_CHAT_PROMPTS: Dict[str, str] = {
    ...
    "complete_app_data": (
        "You are AHVI, an intelligent personal lifestyle assistant. Analyze the user's complete app data "
        "across Wardrobe, Calendar, Medicines, Bills, Meals, Workouts, Skincare, and Contacts. Provide a holistic "
        "summary of their status, highlight key upcoming events or pending bills/medicines, and suggest actionable next steps."
    ),
}
...
def _normalize_module_name(module: str) -> str:
    ...
    if value in {"complete_app_data", "all_data", "complete_data"}:
        return "complete_app_data"
...
_MODULE_SUMMARY_INTENTS: Dict[str, tuple] = {
    ...
    "complete_app_data": (
        ...
        "analyze the app complete data",
        "analyze the apps complete data",
        "analyze the app s complete data",
    ),
}
...
def _module_llm_response(
    *,
    ...
    if module_key == "complete_app_data" or "complete data" in user_message.lower():
        user_id = str(user_profile.get("user_id") or user_profile.get("id") or "").strip()
        if user_id:
            try:
                from services.module_summary_service import get_user_complete_app_data_dict
                app_data = get_user_complete_app_data_dict(user_id)
                context_data = {**(context_data or {}), "complete_app_data": app_data}
            except Exception:
                pass
