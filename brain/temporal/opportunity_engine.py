...
idempotency keys (logical transition window + rule versioning), persists them to
OpportunityStore, and notifies subscribers via OpportunityNotifier.
"""
...
class OpportunityEngine:
    """Deterministic opportunity generator with logical idempotency and notifier dispatch."""

    def evaluate_signal(self, signal: TemporalSignal, rule_version: str = "v1") -> Optional[Opportunity]:
        """Evaluate a single TemporalSignal and create an opportunity if eligible."""
        # Use logical transition window (e.g. PREPARATION_WINDOW_OPEN, STARTING_SOON)
        trigger_window = f"logical_win_{signal.signal_type.value.lower()}"
        ...
        if opportunity_store.has_idempotency_key(idempotency_key, user_id=signal.user_id):
            logger.info(
                "AHVI_OPPORTUNITY_ENGINE_SKIP_DUPLICATE key=%s user_id=%s item_id=%s",
                idempotency_key,
                signal.user_id,
                signal.timeline_item_id,
            )
            return None
        ...
        # Persist to OpportunityStore (Appwrite authoritative)
        saved = opportunity_store.save_opportunity(opp)
        if not saved:
            return None
    ...
    def process_signals(self, signals: List[TemporalSignal], rule_version: str = "v1") -> List[Opportunity]:
        """Evaluate a batch of signals and return generated opportunities."""
        created: List[Opportunity] = []
        for sig in signals:
            opp = self.evaluate_signal(sig, rule_version=rule_version)
            if opp:
                created.append(opp)
        return created
